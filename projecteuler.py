...
def problem4():
    ...
    return max(
        x * y
        for (x, y) in itertools.product(range(100, 1000), repeat=2)
        if mathhelper.is_palindromic(x * y)
    )
...
def problem22():
    ...
    """
    with open(
        os.path.join(os.path.dirname(__file__), "data", "p022_names.txt"), "r"
    ) as file:
        data = file.read()

    names = sorted(name.strip('"') for name in data.split(","))
    return sum(
        sum(ord(c) - 64 for c in list(name)) * (idx + 1)
...
def problem23():
    ...
    """
    abundants = [
        x for x in range(1, 28124) if mathhelper.sum_factors(x, proper=True) > x
    ]
    abundants_sum = {
        x + y for (x, y) in itertools.product(abundants, repeat=2) if x + y < 28124
    }
...
def problem29():
    ...
    """
    return len({x ** y for (x, y) in itertools.product(range(2, 101), repeat=2)})
...
def problem30():
    ...
    """
    return sum(
        x for x in range(2, (9 ** 6) + 1) if x == mathhelper.sum_powers_of_digits(x, 5)
    )
...
def problem31():
    ...
    return_value = 1
    limit = 2
    for p_100 in range(limit, -1, -1):
        limit = 4 - 2 * p_100
        for p_50 in range(limit, -1, -1):
            limit = 10 - 5 * p_100 - 2 * p_50
            for p_20 in range(limit, -1, -1):
                limit = 20 - 10 * p_100 - 5 * p_50 - 2 * p_20
                for p_10 in range(limit, -1, -1):
                    limit = 40 - 20 * p_100 - 10 * p_50 - 4 * p_20 - 2 * p_10
                    for p_5 in range(limit, -1, -1):
                        limit = (
                            101
                            - 50 * p_100
                            - 25 * p_50
...
def problem36():
    ...
    return sum(
        ...
        if mathhelper.is_palindromic(x) and mathhelper.is_palindromic(int(bin(x)[2:]))
    )
...
def problem37():
    ...
    for prime in mathhelper.prime_list(None):
        ...
        for truncate in range(1, len(prime_text)):
            ...
            if not mathhelper.is_prime(left_part) or not mathhelper.is_prime(
                right_part
            ):
                break
...
def problem67():
    ...
    """
    with open(
        os.path.join(os.path.dirname(__file__), "data", "p067_triangle.txt"), "r"
    ) as file:
        data = file.read()
